...
c The 'str' argument must be 64 bytes in size
         ...
         subroutine cvms_query(nn,rlon,rlat,rdep,alpha,beta,rho,ecode)
...
c PES 2011/02/04 check that nn is less than ibig
      if ( nn .gt. ibig )then
         ecode = 1
         goto 98
      endif

c PES 2011/02/04 convert depth meters to feet
      do i = 1, nn
...
c        call postpro
c------------- return points and values---------------
c PES 2011/02/04 convert depth feet back to meters
 97   do i = 1, nn
        rdep(i) = rdep(i) / 3.2808399
      end do
 98   return
...
c RWG 2007/04/04
c          open(17,file=aname2,status='old',err=299)
          fname=rpath(1:loc1)//'/'//aedname(i)//asuf2
          open(17,file=fname,status='old',err=299)
...
c RWG 2007/04/04
c          open(18,file=aname3,status='old',err=259)
          fname=rpath(1:loc1)//'/bmod_edge'
          open(18,file=fname,status='old',err=259)
...
c read file
c RWG 2007/04/04
c           open(12,file=fileii,status='old',err=977)
           fname=rpath(1:loc1)//'/lab_geo2_geology'
           open(12,file=fname,status='old',err=977)
...
c RWG 2007/04/04
c          open(16,file=aname2,status='old',err=99)
          fname=rpath(1:loc1)//'/'//aedname(i)//asuf//a18
...
c
c RWG 2007/04/04
c        open(16,file=filei4,status='old',err=99)
c
        fname=rpath(1:loc1)//'/laup_sur2'
        open(16,file=fname,status='old',err=99)
...
c
c RWG 2007/04/04
c        subroutine postpro
        subroutine postpro(nn,rlon,rlat,rdep,alpha,beta,rho)
         dimension rlon(nn),rlat(nn),rdep(nn),alpha(nn),beta(nn),rho(nn)
...
c RWG 2007/04/04
c          open(16,file='salton_base.sur',status='old',err=2199)
          fname=rpath(1:loc1)//'/salton_base.sur'
          open(16,file=fname,status='old',err=2199)
...
c RWG 2007/04/04
c         open(17,file='impva.edge',status='old',err=2599)
         fname=rpath(1:loc1)//'/impva.edge'
         open(17,file=fname,status='old',err=2599)
...
c RWG 2007/04/04
c         open(17,file='ivmod.edge',status='old',err=2599)
         fname=rpath(1:loc1)//'/ivmod.edge'
         open(17,file=fname,status='old',err=2599)
...
c RWG 2007/04/04
c         open(19,file='eh.modPS',status='old',err=2999)
         fname=rpath(1:loc1)//'/eh.modPS'
...
c RWG 2007/04/04
c         open(15,file=fileib,status='old',err=2978)
         fname=rpath(1:loc1)//'/boreholes'
...
c RWG 2007/04/04
c         open(12,file=fileig,status='old',err=2977)
         fname=rpath(1:loc1)//'/soil_generic'
...
c RWG 2007/04/04
c        open(16,file=filesb,status='old',err=5977)
        fname=rpath(1:loc1)//'/soil.pgm'
...
c RWG 2007/04/04
c        open(16,file=filemo,status='old',err=3233)
        fname=rpath(1:loc1)//'/moho_sur'
        open(16,file=fname,status='old',err=3233)
...
c RWG 2007/04/04
c         open(29,file='3D.out',status='old',err=2998)
         fname=rpath(1:loc1)//'/3D.out'
         open(29,file=fname,status='old',err=2998)
